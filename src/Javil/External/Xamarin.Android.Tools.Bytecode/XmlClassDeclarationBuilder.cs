			...
			return new XElement ("typeParameters",
					typeParameters.Select (tp =>
						new XElement ("typeParameter",
							...
							tp.InterfaceBounds.Count == 0 ? null : new XAttribute ("interfaceBounds",  string.Join (":", tp.InterfaceBounds.Select (_ => SignatureToGenericJavaTypeName (_)))),
                            tp.InterfaceBounds.Count == 0 ? null : new XAttribute ("jni-interfaceBounds",  string.Join (":", tp.InterfaceBounds)))));
		}
